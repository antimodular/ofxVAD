...
public:
	...
	VectorXd fft_calc(double* fft_input){
        ...
        kiss_fft_cfg cfg = kiss_fft_alloc(this->winSize, false, 0, 0);

        kiss_fft_cpx * kiss_input = (kiss_fft_cpx*)malloc(sizeof(kiss_fft_cpx) * this->winSize);
        for (int i = 0; i < this->winSize; i++) {
            kiss_input[i].r = fft_input[i];
        }

        kiss_fft_cpx * kiss_result = (kiss_fft_cpx*)malloc(sizeof(kiss_fft_cpx) * this->winSize);

        kiss_fft(cfg, kiss_input, kiss_result);
        for (int i = 0; i < this->NFFT2; i++) {
            result(i) = sqrt(pow(kiss_result[i].r, 2) + pow(kiss_result[i].i, 2));
        }
        return result;
